...
// Mirrors every color this module paints into signalrgb_colors[] so the
// split-half sync in the keymap (signalrgb_split.c) can forward whatever
// lands on remote-half indices to the other MCU. rgb_matrix_set_color()
// alone never leaves the local half — see keymaps/*/signalrgb_split.c.
static inline void signalrgb_paint(uint8_t index, uint8_t r, uint8_t g, uint8_t b) {
    rgb_matrix_set_color(index, r, g, b);
    signalrgb_colors[index] = (RGB){r, g, b};
}
#endif
...
void led_streaming(uint8_t *data) //Stream data from HID Packets to Keyboard.
{
    ...
    for (uint8_t i = 0; i < numberofleds; i++)
    {
      ...
      #elif defined(RGB_MATRIX_ENABLE)
      signalrgb_paint(index + i, 255, 255, 255);
      #endif
